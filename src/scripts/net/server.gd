...
func receive_chat_message(text: String) -> void:
	...
	if text == "":
		return
	if text.length() > GDTChat.MAX_MESSAGE_LEN:
		return
...
func node_update_request(scene_path: String, node_path: NodePath, property_dict: Dictionary) -> void:
	...
	if not GDTValidator.is_path_safe(scene_path):
		return
	if not id_has_permission(id, GodotTogether.Permission.EDIT_SCENES):
		return
...
func node_rename_request(scene_path: String, old_path: NodePath, new_name: String) -> void:
	if not GDTValidator.is_path_safe(scene_path):
		return
	if not id_has_permission(multiplayer.get_remote_sender_id(), GodotTogether.Permission.EDIT_SCENES):
		return
...
func file_add_from_client(path: String, buffer: PackedByteArray) -> void:
	...
	if not id_has_permission(id, GodotTogether.Permission.ADD_CUSTOM_FILES):
		return
...
func file_modify_from_client(path: String, buffer: PackedByteArray) -> void:
	...
	if not id_has_permission(id, GodotTogether.Permission.MODIFY_CUSTOM_FILES):
		return
	if not GDTValidator.is_path_safe(path):
		return
...
func file_remove_from_client(path: String) -> void:
	...
	if not id_has_permission(id, GodotTogether.Permission.DELETE_SCRIPTS):
		return
	if not GDTValidator.is_path_safe(path):
		return
